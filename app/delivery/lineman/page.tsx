...
export default function LineManPage() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      {/* ส่วนหัวคุมโทนสี Butter & Green (#013E37) พร้อมตัวเลือกเดือน */}
      <div 
      ...
      >
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#FFEFB3' }}>🛵 สรุปยอดขาย: LINE MAN</h1>
          <p className="text-sm text-emerald-100 mt-1">แสดงข้อมูลและยอดสรุปเฉพาะแพลตฟอร์ม LINE MAN แยกตามเดือน</p>
        </div>
      ...
      {/* 2 การ์ดหลักใหญ่เด่นชัด: ยอดขายหน้า App (Gross) & ยอดรับสุทธิ (Net) ปรับสลับสีตามหน้า Grab */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {/* ยอดขายหน้า App (Gross) - พื้น Butter Yellow (#FFEFB3) */}
        <div 
          className="p-6 rounded-2xl shadow-md border-2 flex flex-col justify-between"
          style={{ backgroundColor: '#FFEFB3', borderColor: '#013E37' }}
        >
          <p className="text-sm font-bold uppercase tracking-wide" style={{ color: '#013E37' }}>📦 ยอดขายหน้า App (Gross)</p>
        ...
        {/* ยอดรับสุทธิ (Net) - พื้น Dark Green (#013E37) */}
        <div 
          className="p-6 rounded-2xl shadow-md border-2 flex flex-col justify-between"
          style={{ backgroundColor: '#013E37', borderColor: '#FFEFB3' }}
        >
          <p className="text-sm font-bold uppercase tracking-wide" style={{ color: '#FFEFB3' }}>💰 ยอดรับสุทธิ (Net)</p>
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-4xl font-black text-white">{totalNet.toLocaleString()}</span>
            <span className="text-base font-semibold text-emerald-200">บาท</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
          <h2 className="font-bold text-base" style={{ color: '#013E37' }}>ประวัติรายการ LINE MAN ({lineManIncomes.length} รายการรายรับ)</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="text-xs uppercase tracking-wider text-white" style={{ backgroundColor: '#013E37' }}>
              ...
              {lineManIncomes.length > 0 ? (
                lineManIncomes.map((item, index) => {
                  ...
                  return (
                    <tr key={index} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="p-4 font-medium text-slate-800">{item.channel || item.platform || 'LINE MAN'}</td>
                      ...
                      <td className="p-4 text-right font-bold" style={{ color: '#013E37' }}>{net.toLocaleString()}</td>
                    </tr>
                  );
                })
              ) : (
                <tr>
                  <td colSpan={6} className="p-10 text-center text-slate-400">
                    ยังไม่มีข้อมูลการขายของ LINE MAN ในเดือนที่เลือก
                  </td>
                </tr>
              )}
            </tbody>
